...
def heuristic(a, b):
    """Euclidean distance heuristic function for 8-direction movement"""
    return np.sqrt((a[0] - b[0])**2 + (a[1] - b[1])**2)

def astar(env, start, end):
    """Perform A* pathfinding algorithm with 8 directions"""
    ...
    while open_list:
        ...
        for direction in DIRECTIONS:
            ...
            if (0 <= neighbor_pos[0] < env.shape[0] and
                ...
                # Check if neighbor is already in open list with a better f score
...
def visualize_path(env, path):
    ...
    plt.title("A* Pathfinding with Arrows")
    plt.show()
...
if __name__ == "__main__":
    ...
    path, explored_nodes, total_cost, exec_time = astar(env, start, end)
