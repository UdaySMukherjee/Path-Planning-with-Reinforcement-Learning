...
def visualize_training_progress(env, rewards, title, episode_num):
    plt.plot(rewards)
    ...
    plt.grid(True)
    plt.show()
    env.render(episode_num=episode_num)
...
def run_qlearning_episodes(env, agent, episodes=100):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            ax.set_title(f"Q-Learning - Ep {ep+1}, Step {step_count}, Eps {agent.epsilon:.2f}")
            plt.draw()
            plt.pause(0.01)
...
if __name__ == "__main__":
    ...
    agent = QLearningAgent(state_size=2, action_size=env.num_actions)
    rewards = run_qlearning_episodes(env, agent, episodes=50)
    visualize_training_progress(env, rewards, "Q-Learning", episode_num="final_qlearning")